...
```{r}
#TO-DO
indices = sample(1:nrow(bills_data))
X = bills_data[indices, c("total", "days", "invoice_date")]
y = bills_data[indices, "paid"]
X_test = X[1:100, ]
y_test = y[1:100, ]
X_train = X[101:200, ]
y_train = y[101:200, ]
mod = YARFCART(X = X_train, y = y_train)
y_hat_train = predict(mod, X_train)
y_hat_test = predict(mod, X_test)
oos = table(y_test, y_hat_test)
oos
FDR = oos[1, 2] / sum(oos[ , 2])
FOR = oos[2, 1] / sum(oos[ , 1])
```
...
M = 1000
for (m in 1 : M) {
  mod_bag = YARFBAG(Boston_train, Boston_train$medv, num_trees = m, calculate_oob_error = FALSE)
  y_hat_bag = predict(mod_bag, Boston_test)
  s_e_bag = sd(Boston_test$medv - y_hat_bag)
}
