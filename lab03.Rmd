...
```{r}
#max(table(y))
#g = c("a","a","b",1,"c",1,1,"f","f",3,2,"a",5,6)
#b = table(g)
#b
#b[b == max(b)]
mode = function(v){
...
```{r}
#' TO-DO: PerceptronFunction
#'      
#' TO-DO: This function implements the perceptron learning algorithm which is a supervised learning of binary classifiers. It is a type of linear classifier, i.e. a classification algorithm that makes all of its predictions based on a linear predictor function combining a set of weights with the feature vector
#'
#' @param Xinput      TO-DO: The Xinput is a vector of all our inputs or coordinates of our x and y or x1 and x2 
#' @param y_binary    TO-DO: This is going to be the vector of our output essentially this is going to be either 1 or 0 depending on how we set up our line and what we consider what falls under what in better words how we classify our inputs.
#' @param MAX_ITER    TO-DO: The amount of times the algorithm will run before stopping
#' @param w           TO-DO: This is our bias 
#'
...
perceptron_learning_algorithm = function(Xinput, y_binary, MAX_ITER = 1000, w = NULL){
  #TO-DO
  #intialize input with w0 as 1's remember w0 + w1*x1 + w2*x2
  ...
  for(iter in 1:MAX_ITER){
    for(i in 1 : nrow(Xinput)){
      ...
      y_i = y_binary[i]
      #Okay the only i think can think of making it work for all w_vec ith iterations is to do another for loop which seems super in-efficient 
...
You have a two feature first being x and second being y axis respectively, where each response are plotted on the graph given those two feature. 
...
TO-DO
I believe the -7 is our intercept.
4 is the slope which can be seen on the bottom code/plot
And -1 is our bias or offset
...
TO-DO 
The line could have been better I dont like the fact the line is directly going through the right-most orange response. Ideally it could have been seperated without touching either responses but it does seperate nevertheless.
...
TO-DO
I believe it ism it is a more satisfying line as it clearly seperates both responses without touching a response unlike the perceptron
...
linear_svm_learning_algorithm = function(Xinput, y_binary, MAX_ITER = 5000, lambda = 0.1){
  ...
  #candidateClosestPair = {closest pair of opposite classes}
  #while there are no points violating:
  #   Find any points which are violating
  #   candidateClosestPair = candidateClosestPair + violator
  #               S
  #   violating points
  #   if any any point is < 0 due to adding c to S then
  #       candidateClosestPair = candidateClosestPair\p
  #       repeat till all points are pruned
  #   endif
  #end while
}
```
...
```{r}
#TO-DO
data.a = c(2,4,1,3,5,7,4,8,3,2,1,7)

Xtest = matrix(data.a, nrow = nrow(X_simple_feature_matrix), ncol = ncol(X_simple_feature_matrix))
Xtest
nn_algorithm_predict(X_simple_feature_matrix,y_binary,Xtest)
nn_algorithm_predict(X_simple_feature_matrix,y_binary,c(1,2,3,5,8,4,2,4,5,6,3,2,3,2))
nn_algorithm_predict(X_simple_feature_matrix,y_binary,45)

((X_simple_feature_matrix[,1] - Xtest[,1])^2)
