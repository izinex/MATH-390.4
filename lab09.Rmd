...
mean(ytrain * log(phatTrain) + (1 - ytrain) * log(1 - phatTrain)) #log score computation
mean(-(ytrain - phatTrain)^2) #brier score computations
```
...
repeat {
  ...
  phatSelect = suppressWarnings(predict(logistic_mod, Xmm_select[, predictor_by_iteration, drop = FALSE], type = 'response'))

  oos_brier = mean(-(yselect - phatSelect)^2) 
...
simulation_results = data.frame( iteration=1:length(in_sample_brier_by_iteration), 
...
ggplot(simulation_results)+
  geom_line(aes(x=iteration,y=in_sample_brier_by_iteration),col='green')+
  geom_line(aes(x= iteration, y=oos_brier_by_iteration),col='red')+
  ylab("Brier score")
```
...
```{r}
#TO-DO
max_p_opt = which.max(oos_brier_by_iteration)
Xmm_sub = Xmm_train[, colnames(Xmm_train)[predictor_by_iteration[1 : max_p_opt]], drop = FALSE]
mod = suppressWarnings(glm(ytrain ~ ., Xmm_sub, family = "binomial"))
mod$coefficients[order(abs(logistic_mod$coefficients),decreasing = TRUE )][1:5]
...
```{r}
#TO-DO
phatTrain = predict(mod, Xmm_train[predictor_by_iteration[1:max_p_opt]], type = 'response')
mean(-(ytrain - phatTrain)^2) 
phatSelect = predict(mod, Xmm_select[predictor_by_iteration[1:max_p_opt]], type = 'response')
mean(-(yselect - phatSelect)^2) 
phatTest = predict(mod, Xmm_test[predictor_by_iteration[1:max_p_opt]], type = 'response')
...
```{r}
#TO-DO
ggplot(data.frame(phatTest = phatTest,ytest = factor(ytest))) + 
  geom_histogram(aes(x = phatTest, fill = ytest))
```
...
```{r}
#TO-DO
phat_threshold = ifelse(phatTest<.5,0,1)
P = sum(ytest==1)
N = sum(ytest==0)
TP = sum(phat_threshold==1 &ytest==1)
TN = sum(phat_threshold==0 &ytest==0)
FN = sum(phat_threshold==0 & ytest==1)
FP= sum(phat_threshold==1 & ytest==0)
sensitivity = TP/P
specificity = TN/N
FDR = FP/(FP+TP)
FOR = FN/(FN+TN)

paste0("misclassification error: ",mean(ytest!=phat_threshold))
...
compute_metrics_prob_classifier = function(p_hats, y_true, res = 0.001){
  ...
  for (i in 1 : length(p_thresholds)){
    p_th = p_thresholds[i]
    # yhats
    phat_threshold[i] = ifelse(p_hats[i] >= p_th, 1, 0)
  
    TN[i] = ifelse(phat_threshold[i] == 0 & y_true[i] == 0, 1, 0)
    FN[i] = ifelse(phat_threshold[i] == 0 & y_true[i] == 1, 1, 0)
    TP[i] = ifelse(phat_threshold[i] == 1 & y_true[i] == 1, 1, 0)
    FP[i] = ifelse(phat_threshold[i] == 1 & y_true[i] == 0, 1, 0)
...
performance_metric = compute_metrics_prob_classifier(phatSelect, yselect)
head(performance)
c_FP = -1
c_FN = 3 * c_FP
...
trapz(performance_metric$FPR, performance_metric$recall)
...
```{r}
#TO-DO
total_cost = performance_metric$cost =  performance_metric$p_th*performance_metric$TN+ (1-performance_metric$p_th)*performance_metric$TP
total_cost
...
data(storms)
storm_df = storms %>%
  filter(!is.na(ts_diameter)|!is.na(hu_diameter!=0))
storm_df
```
...
```{r}
#TO-DO
long_storm_df = storms_df %>% 
  gather(diameter_type, diameter, ts_diameter:hu_diameter) %>% 
  mutate(diameter_type = ifelse(diameter_type == "ts_diameter", "hu", "ts"))
```
...
```{r}
#TO-DO
four_random_storms = sample(storms$name, 4)
