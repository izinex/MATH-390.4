...
```{r}
#TODO
s_e_in_sample = sd(y-y_train)
s_e_oos = sd(y-y_test)
s_e_in_sample
s_e_oos
#out of sample error is greater
...
s_e_in_sampleT = s_e_in_sampleT + sd(y-y_train)
s_e_oosT = s_e_oosT + sd(y-y_test)
}

abs(s_e_in_sampleT/1000 - s_e_oosT/1000)
...
Xjunk_train = Xjunk[index_train, ]
y_train = y[index_train]
Xjunk_test = Xjunk[index_test, ]
y_test = y[index_test]
s_es = vector()
for(i in 2:ncol(Xjunk)){
  mod = lm(y_train ~ ., data.frame(Xjunk_train[ , 1:i]))
  s_es =- c(s_es, sd(mod$residuals))
}
...
for (i in 2:ncol(Xjunk_train)){
  ...
  y_hat_oos <- predict(mod, data.frame(Xjunk_test[ , 1:i]))
  oos_residuals <- y_test - y_hat_oos
  
  oos_s_e <- c(oos_s_e, sd(oos_residuals))
}
...
#TODO
as.matrix(X2dummy[,1])
x2dummyfeatures = as.matrix(X2dummy[,2, drop = FALSE])
x2dummyfeatures
```
...
#TODO
xdum = X[,3]
x3_bin = cut(xdum, breaks = (quantile(xdum, c(0,.5,1))), include.lowest=TRUE)
Xidummy = factor(model.matrix(~ 0 + ., data.frame(x3_bin)))
xiifacotor = factor(xdum)
Xidummyfeatures = as.matrix(xiifacotor[,2, drop = FALSE])

Xidummyfeatures = as.matrix(Xidummy[,2, drop = FALSE])
Xdummy = cbind(Xdummy, xdum)

Xdummy = x2dummyfeatures
Xdummy
for (i in 2:14){
  xdum = X[,1]
  x_bin = cut(xdum, breaks = unique(quantile(xdum, c(0,.5,1))), include.lowest=TRUE)
  Xidummy = model.matrix(~ 0 + ., data.frame(x_bin))
  Xidummyfeatures = as.matrix(Xidummy[,2, drop = FALSE])

  Xdummy = cbind(Xdummy, Xidummyfeatures)
}
