...
b0 is the intercept, b1 is the change in height for the children,
RMSE is the variation of the children heights, and 
R^2 is a indicator of how good our model is

How good is this model? How well does it predict? Discuss.
This model is reasonable because it gives a good estimate of a child's height given its parents with some level of error
TO-DO
...
It is reasonable because height is genetic, children inherit their parent's DNA which dictates their height.

TO-DO

If they were to have the same height and any differences were just random noise with expectation 0, what would the values of $\beta_0$ and $\beta_1$ be?

b0 and b1 would be be 0 if they were the same height
...
Because the offspring always went towards to the mean

TO-DO

Why should this effect be real?

While I am not sure if this data or study is a direct representation of reality but it could make sense because everything tends to go to the average if historically offsprings ancestors were tall but their parents were short it might in our DNA to make sure offspring always deviate towards the mean of historical average height 

TO-DO

You now have unlocked the mystery. Why is it that when modeling with $y$ continuous, everyone calls it "regression"? Write a better, more descriptive and appropriate name for building predictive models with $y$ continuous.

When working with continous y in models it seems it will always deviates towards the mean according to Galton's studies thus it always regresses 
TO-DO
...
```{r}
#TO-DO
X = model.matrix(Petal.Length ~ Species, data = iris)
X

X = model.matrix(Petal.Length ~ Species + 0, data = iris)
X
```
